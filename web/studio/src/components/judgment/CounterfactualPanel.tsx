...
export function CounterfactualPanel({ judgment }: { judgment: JudgmentTraceViewModel }) {
  ...
  const hasUntrusted = judgment.fact_nodes.some((fact) => fact.namespace === "source" && String(fact.value) === "untrusted");
  ...
  const lines = [
    ...
    hasSecret ? "如果没有 asset.touched_type=secret_file：INV-SECRET 类安全门可能不命中，决策可能降为沙箱或审批。" : "当前没有 secret_file 资产事实，系统不会把它解释为密钥资产硬阻断。",
